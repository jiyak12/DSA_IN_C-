...
// brute force- iterate through the array and store all the non zero numbers then put all the non zero numbers to the front and put all the zeros at the remaining places in the array
 /* optimal approach -First, using a loop, we will place the pointer j. If we don’t find any 0, we will not perform the following steps.
