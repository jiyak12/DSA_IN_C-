...
// in the two arrays there can be dupes but the unified array won't contain any dupes.
/* Take two pointers i,j pointing to the 0th index of arr1 and arr2.
Create an empty vector for storing the union of arr1 and arr2.
the union is nothing but the distinct elements in arr1 + arr2 
traverse the arr1 and arr2 using pointers i and j and insert the distinct elements found into the union vector.
 arr1[ i ] == arr2[ j ] 
